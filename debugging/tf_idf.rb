...
# The issue is on line 26, one of the values needs to be a float in order to be calculated correctly, as seen below
def idf(term, documents)
  number_of_documents = documents.length
  number_of_documents_with_term = documents.count { |d| tf(term, d) > 0 }
...
# We also need to round the final return to get the expected output
def tfidf(term, document, documents)
